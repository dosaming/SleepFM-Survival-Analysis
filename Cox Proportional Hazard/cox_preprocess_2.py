...
def main():
    ...
    if isinstance(data, dict) and "X" in data:
        ...
        # 혹시 X가 torch.Tensor나 리스트로 들어왔을 수 있으니 정리
        if isinstance(Z, list) or isinstance(Z, tuple):
            # 리스트인데 3모달(stacked)일 수 있음
            # modality에 따라 골라 쓰기
            mod_map = {
                "respiratory": 0,
                "sleep_stages": 1,
            ...
            if args.modality in mod_map and len(Z) >= mod_map[args.modality] + 1:
                Z = Z[mod_map[args.modality]]
            elif args.modality == "combined" and len(Z) >= 3:
                # [N,512] 3개를 concat → [N,1536]
                Z = np.concatenate([np.array(z) for z in Z[:3]], axis=1)
            else:
                raise ValueError("dict 포맷 embeddings에서 X가 리스트인데 modality 매칭이 안 됩니다.")
    ...
    # (2) 리스트/튜플 포맷: [resp_emb, sleep_emb, ekg_emb]
        ...
        if args.modality in mod_map:
            idx = mod_map[args.modality]
            if len(emb_list) <= idx:
                raise ValueError(f"embeddings_pickle 리스트 길이={len(emb_list)}인데 {args.modality}용 index {idx}가 없습니다.")
            Z_raw = emb_list[idx]
            ...
            if len(emb_list) < 3:
                raise ValueError("combined modality인데 embeddings 리스트에 3개 모달리티가 없습니다.")
            arrs = []
            for z in emb_list[:3]:
                if isinstance(z, torch.Tensor):
                    arrs.append(z.cpu().numpy())
                else:
                    arrs.append(np.array(z))
            Z = np.concatenate(arrs, axis=1)
        else:
            raise ValueError("지원하지 않는 modality 조합입니다 (리스트 포맷).")

        ids = None  # 리스트 포맷에는 보통 id 정보가 없다고 가정

    else:
        raise ValueError("알 수 없는 embeddings_pickle 포맷입니다. dict 또는 list/tuple이어야 합니다.")
    ...
    if ids is None or any(
        v is None for v in (ids.tolist() if isinstance(ids, np.ndarray) else ids)
    ):
        ...
        # 원래 data가 dict면 ids까지 inject 해서 patched 버전도 저장
    ...
    # 4) 이벤트/검열 테이블
    events_df = preprocess_events(args.events_csv, time_origin=args.time_origin)
