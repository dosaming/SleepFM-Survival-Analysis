...
def parallel_prepare_data(args):
    ...
    for mrn in tqdm(mrns):
        ...
        if mrn in mrn_train:
            ...
            logger.warning(f"{mrn} not in any split")
            continue
...
def main():
    ...
    assert len(mrns) >= 991, f"MRN 수가 부족합니다. 현재: {len(mrns)}명"

    # 고정된 수로 split
    random.seed(random_state)
    random.shuffle(mrns)
    #physionet: 742, 50, 199 
    mrn_train = set(mrns[:742])
    mrn_valid = set(mrns[742:742+50])
    mrn_test = set(mrns[742+50:742+50+199])
    ...
    tasks = [(mrns_one_thread, dataset_dir, mrn_train, mrn_valid, mrn_test) for mrns_one_thread in mrns_per_thread]
