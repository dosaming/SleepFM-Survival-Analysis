...
def extract_labels(path):
    ...
    label_keys = ['W', 'N1', 'N2', 'N3', 'R']
    length = data['data']['sleep_stages']['W'].shape[1]
    labels = np.zeros((length, len(label_keys)))
...
def main():
    ...
    subject_dirs = glob.glob(os.path.join(path_to_edf_files, "training", "*"))
    subject_dirs = [d for d in subject_dirs if os.path.isdir(d) and ".ipynb_checkpoints" not in d]
    ...
    for subject_path in subject_dirs:
        if not os.path.isdir(subject_path):
            continue
        if "RECORDS" in subject_path or "ANNOTATORS" in subject_path:
            continue
