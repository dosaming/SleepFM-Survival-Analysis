...
mp.set_start_method("spawn", force=True)           # 포크 대신 spawn
torch.multiprocessing.set_sharing_strategy("file_system")  # /dev/shm 대신 tmp 파일
...
from config_shhs_125 import CHANNEL_DATA_IDS, LABELS_DICT
...
def main():
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=4,
        pin_memory=False,             # CPU RAM 부담 줄임
        persistent_workers=True,      # 워커 재사용 (에폭마다 재생성 방지)
        prefetch_factor=1,            # 워커당 미리 불러올 배치 수 ↓
